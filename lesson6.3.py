...
def execute_updates(index, offset, omega updates):
    omega[index][index] += updates[0]
    omega[index][index+offset] += updates[1]
    omega[index+offset][index] += updates[2]
    omega[index+offset][index+offset] += updates[3]
def update_xi_values(current, next, z, xi):
...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    omega = [[ 0.0 for x in range(N + num_landmarks)] for y in range(N + num_landmarks)]
    updates = [1, -1 ,-1 ,1]
    xi = [[ 0.0 for i in range(N + num_landmarks)] for j in range(2)]
    for row in range(len(data)):
        landmarks = data[i][0]
        moves = data[i][1]
        execute_updates(row, 1, omega, updates)
        update_xi_values(row, 1, moves, xi)
        for i in range(len(landmarks)):
            execute_updates(N, landmarks[i][0], omega, updates)
            update_xi_values(row, N+landmarks[i][0], [landmarks[i][1], landmarks[i][2]], xi)
...
test_data1 =
[
 [[[1, 19.457599255548065, 23.8387362100849], [2, -13.195807561967236, 11.708840328458608], [3, -30.0954905279171, 15.387879242505843]], [-12.2607279422326, -15.801093326936487]]
